...
$$e(\boldsymbol{s}_{ij}) = \bar{y}(\boldsymbol{s}_j) - y(\boldsymbol{s}_{ij})$$

A equação acima mostra que o resíduo, $e(\boldsymbol{s}_{ij})$, de uma observação espacial qualquer, $y(\boldsymbol{s}_{ij})$, localizada na categoria $j$, é dado pela diferença entre a média daquela categoria, $\bar{y}(\boldsymbol{s}_j)$, e o valor da respectiva observação.
