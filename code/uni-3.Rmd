...
ambiental explicada estocasticamente, nesse caso usando um campo Gaussiano estacionário e autocorrelacionado, cuja média é igual a zero.

A partir do conjunto finito de observações espaciais da variável ambiental, 
$\boldsymbol{y}^\text{T} = \{y(\boldsymbol{s}_1), y(\boldsymbol{s}_2), \ldots, y(\boldsymbol{s}_n)\}$, nossa inteção é estimar os coeficientes da regressão linear, $\boldsymbol{\beta}$, os parâmetros do variograma, 
$\boldsymbol{\theta}^\text{T} = (\tau^2, \sigma^2, \alpha)$, a fim de que seja possível predizer 
