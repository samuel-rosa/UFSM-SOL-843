...
pts <- sp::spsample(pedologia25, n = 50, type = "regular", offset = c(0.5, 0.5))
col_soil_um <- terrain.colors(nlevels(pedologia25$um))
sp::spplot(
  pedologia25, col.regions = col_soil_um, alpha.regions = 0.3,
  main = "Malha amostral regular", colorkey = FALSE, sub = glue('n = {length(pts)}')) +
  lattice::xyplot(x2 ~ x1, data = pts@coords %>% as.data.frame(), col = 'red', pch = 17) %>% 
